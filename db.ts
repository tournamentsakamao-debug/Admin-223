...
export const DB = {
  ...
  register: async (username, password): Promise<{ success: boolean; message: string }> => {
    ...
    if (cleanUsername === ADMIN_USERNAME) {
      return { success: false, message: "Username restricted." };
    }
    ...
    return error ? { success: false, message: "Registration failed." } : { success: true, message: "Registered Successfully!" };
  },
  ...
  toggleUserBlock: async (uid: string) => {
    ...
    const { data: target } = await supabase.from('users').select('is_blocked').eq('id', uid).single();
  ...
  updateUserBalance: async (userId: string, amount: number) => {
    ...
    // Security layer: only admin or self-payment allowed
  ...
  createJoinRequest: async (data: any, payViaWallet: boolean) => {
    const user = DB.getCurrentUser();
    if (!user || user.is_blocked) return { success: false, message: "Blocked." };
    ...
    if (payViaWallet) {
      ...
      return { success: true, message: "Joined Arena!" };
    }
    ...
    return error ? { success: false, message: "Submission failed." } : { success: true, message: "Proof Uploaded." };
  },
  ...
  requestWithdrawal: async (userId: string, username: string, amount: number, upiId: string) => {
    ...
    await supabase.from('withdrawals').insert([{
      user_id: userId, username, amount: safeAmount, upi_id: upiId, timestamp: Date.now(), status: 'PENDING'
    }]);
    return true;
  },
  ...
  requestWalletAdd: async (userId: string, amount: number, utr: string) => {
    const { error } = await supabase.from('wallet_adds').insert([{
      user_id: userId, amount: parseCleanNumber(amount), utr, timestamp: Date.now(), status: 'PENDING'
    }]);
    return !error;
  },
  ...
  setWinner: async (tId: string, winnerUserId: string, prize: string) => {
    ...
    await supabase.from('tournaments').update({ status: 'COMPLETED', winner_id: winnerUserId }).eq('id', tId);
